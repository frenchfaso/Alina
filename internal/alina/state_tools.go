...
func (e *Engine) stateTool(j *runningJob, c ToolCall) (string, error) {
	if c.Name == "memory" {
		...
		case "read":
			...
			return jsonText(r), err
		case "note":
			id, err := e.Memory.Note(j.ctx, time.Now(), j.Session, j.ID, a.Kind, a.Text, a.Supersedes, a.Sources)
			return jsonText(map[string]string{"id": id}), err
		case "focus":
			err := e.Memory.Focus(j.ctx, a.ID, a.Pinned, time.Now())
